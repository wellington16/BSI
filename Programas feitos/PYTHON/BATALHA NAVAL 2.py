...
while continuar == "S":
    ...
    def  configuracao(d):
        dimensao=(input("Deseja configurar? Se sim digite 'S',se 'N' :"))
        while dimensao!= "N":
            arqaleatorio= open("C:\Users\WELLINGTON\Downloads\config.txt")
            ler1 = arqaleatorio.read()
            tamanho=Tamanho
            navios = QuantidadeNavios
            tiros = QuantidadeErros
            arqaleatorio.close()
            
        else:
            arqaleatorio=open("C:\Users\WELLINGTON\Downloads\grid.txt",)
            ler2 = a.read()
