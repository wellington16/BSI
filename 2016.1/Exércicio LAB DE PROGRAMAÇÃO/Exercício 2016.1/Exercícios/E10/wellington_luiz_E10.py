...
'''
    ...
    Exercício 10 - Laboratório de Programação
    título: Notação Polonesa.
...
#arqEntrada = open("in.txt", "r")
#arqSaida = open("out.txt", "w")
...
while True:
    ...
    for operador in range(len(expressao)-1,-1,-1):
        
        #Se for Vezes calcule
        if (expressao[operador] == '*'):
            op1 = int(operandos.desempilhar())
            op2 = int(operandos.desempilhar())
            #print("operando 1 : ",op1, "operando 2 : ", op2)
            result = int(op1)* int(op2)
            operandos.InserirNoFim(result)
        ...
        #Se não for nem das alternativas acima, então faça, adicione
        #os operandos.
        else:
            operandos.InserirNoFim(expressao[operador])
            
    resultado = operandos.desempilhar()
    print("O resultaodo é => " , resultado)
